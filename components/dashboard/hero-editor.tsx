...
export function HeroEditor({ initialContent }: { initialContent: HeroContent }) {
  ...
  function removeSlide(id: number) {
    setContent((current) => ({ ...current, slides: current.slides.filter((slide) => slide.id !== id) }))
  }
        ...
        {content.slides.map((slide, index) => (
          <div key={slide.id} className="rounded-[1.75rem] border border-white/80 bg-white/95 p-6 shadow-[0_18px_45px_rgba(15,23,42,0.05)]">
            <div className="mb-4 flex items-center justify-between">
              <Button type="button" variant="ghost" className="rounded-xl text-red-600 hover:text-red-700" onClick={() => removeSlide(slide.id)}>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="md:col-span-2">
                <FileUploadField label="صورة الشريحة" value={slide.image} onChange={(value) => updateSlide(index, "image", value)} />
              </div>
              <div className="space-y-2 text-right">
                <Label htmlFor={`hero-title-${slide.id}`}>العنوان</Label>
                <Input id={`hero-title-${slide.id}`} value={slide.title} onChange={(event) => updateSlide(index, "title", event.target.value)} />
              </div>
              <div className="space-y-2 text-right">
                <Label htmlFor={`hero-subtitle-${slide.id}`}>النص العلوي</Label>
