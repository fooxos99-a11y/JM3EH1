...
const FLOATING_TRIGGER_STORAGE_KEY = "admin-chat-trigger-position"
const FLOATING_TRIGGER_SIZE = 56
const FLOATING_TRIGGER_MARGIN = 24
...
export function AdminChatPanel({ iconOnly = false, triggerClassName = "", side = "left" }: AdminChatPanelProps) {
  ...
  function clampFloatingPosition(nextPosition: FloatingPosition) {
    const maxX = Math.max(FLOATING_TRIGGER_MARGIN, window.innerWidth - FLOATING_TRIGGER_SIZE - FLOATING_TRIGGER_MARGIN)
    const maxY = Math.max(FLOATING_TRIGGER_MARGIN, window.innerHeight - FLOATING_TRIGGER_SIZE - FLOATING_TRIGGER_MARGIN)

    return {
      x: Math.min(maxX, Math.max(FLOATING_TRIGGER_MARGIN, nextPosition.x)),
      y: Math.min(maxY, Math.max(FLOATING_TRIGGER_MARGIN, nextPosition.y)),
    }
  }
  ...
  useEffect(() => {
    ...
    function handleResize() {
      setTriggerPosition((current) => current ? clampFloatingPosition(current) : current)
    }
  ...
  function handleTriggerPointerMove(event: React.PointerEvent<HTMLButtonElement>) {
    if (!iconOnly || !dragStateRef.current || dragStateRef.current.pointerId !== event.pointerId) {
      return
    }
