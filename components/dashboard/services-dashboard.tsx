...
async function embedAssetImageForPdf(pdf: PdfDocumentType, imageUrl: string) {
...
export function ServicesDashboard({ initialTab = "image_to_pdf" }: { initialTab?: string } = {}) {
  ...
  const editTextLayersByPage = useMemo(() => {
    const layersByPage = new Map<number, EditableTextLayer[]>()

    for (const layer of editTextLayers) {
      const pageLayers = layersByPage.get(layer.pageNumber)
      if (pageLayers) {
        pageLayers.push(layer)
      } else {
        layersByPage.set(layer.pageNumber, [layer])
      }
    }
  ...
  const placedAssetsByPage = useMemo(() => {
    const assetsByPage = new Map<number, PlacedAsset[]>()

    for (const placedAsset of placedAssets) {
      const pageAssets = assetsByPage.get(placedAsset.pageNumber)
      if (pageAssets) {
        pageAssets.push(placedAsset)
      } else {
        assetsByPage.set(placedAsset.pageNumber, [placedAsset])
      }
    }
  ...
  const isCompactServiceView = currentTab === "stamps" || currentTab === "image_to_pdf" || currentTab === "pdf_to_images" || currentTab === "pdf_editor" || currentTab === "writer" || currentTab === "compress"
  const isFileConversionView = currentTab === "image_to_pdf" || currentTab === "pdf_to_images"
    ...
    <section className="space-y-6 text-right">
      {isCompactServiceView ? null : (
        <div className="rounded-[1.75rem] border border-white/80 bg-white/95 p-6 shadow-[0_18px_45px_rgba(15,23,42,0.05)]">
          <h1 className="text-2xl font-bold text-foreground">{pageTitleByTab[currentTab] ?? "الخدمات"}</h1>
          <p className="mt-3 text-sm leading-7 text-muted-foreground">
            {pageDescriptionByTab[currentTab] ?? "أدوات داخلية للتعامل مع الملفات والقوالب داخل لوحة التحكم."}
          </p>
        </div>
      )}
      ...
      {isFileConversionView ? (
        <div className="flex justify-end" dir="ltr">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button type="button" variant="outline" className="rounded-xl">تحويل الملفات</Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="min-w-[190px] text-right" dir="rtl">
              <DropdownMenuItem className="justify-end" onClick={() => setCurrentTab("image_to_pdf")}>تحويل إلى PDF</DropdownMenuItem>
              <DropdownMenuItem className="justify-end" onClick={() => setCurrentTab("pdf_to_images")}>تحويل إلى صورة</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      ) : null}
                    ...
                    {editPreviewPages.map((page) => (
                      <div key={page.pageNumber} className="relative mx-auto max-w-3xl overflow-hidden rounded-[1.25rem] border border-white bg-white shadow-[0_18px_40px_rgba(15,23,42,0.08)]">
                        ...
                        >
                          ...
                          {(editTextLayersByPage.get(page.pageNumber) ?? []).map((layer) => (
                            <button
                              key={layer.id}
                              type="button"
                      ...
                      {stampPreviewPages.map((page) => (
                        ...
                        >
                          ...
                          >
                            ...
                            {(placedAssetsByPage.get(page.pageNumber) ?? []).map((placedAsset) => {
                              const asset = assetMap.get(placedAsset.assetId)
                              if (!asset) {
                                return null
                              }
