...
export async function Footer({ content, logo }: { content?: SiteContentMap["footer"]; logo?: SiteContentMap["logo"] } = {}) {
  const resolvedContent = content ?? await getSiteSectionContent("footer")
  const resolvedLogo = logo ?? await getSiteSectionContent("logo")
  const arabicName = resolvedLogo.arabicName.trim() || resolvedContent.organizationName
  const englishName = resolvedLogo.englishName.trim() || "New Muslims Care Association"
    ...
    <footer className="relative border-t border-border/50 bg-gradient-to-b from-[#f8fafa] to-white">
      <div className="container mx-auto px-4 py-16">
        <div className="grid grid-cols-1 gap-10 md:grid-cols-2 lg:grid-cols-4">
          <div className="lg:col-span-1">
              ...
              {resolvedLogo.logo ? (
                <>
                  <div className="flex h-[84px] w-[84px] items-center justify-center px-1 py-3">
                    <Image src={resolvedLogo.logo} alt={resolvedLogo.alt} width={84} height={84} sizes="84px" className="h-full w-full object-contain" />
                  </div>
                  <div className="text-right">
                    <h3 className={`text-lg ${getWeightClass(resolvedLogo.arabicFontWeight)}`} style={{ color: resolvedLogo.textColor }}>{arabicName}</h3>
                    <p className={`mt-1 text-xs tracking-[0.02em] ${getWeightClass(resolvedLogo.englishFontWeight)}`} style={{ color: resolvedLogo.textColor }} dir="ltr">{englishName}</p>
                  </div>
                </>
              ) : (
            ...
            <p className="mb-6 text-sm leading-relaxed text-muted-foreground">{resolvedContent.about}</p>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {resolvedContent.quickLinks.map((link) => (
            ...
            <ul className="space-y-4">
              <li className="group flex items-start gap-3">
                ...
                <div>
                  ...
                  <span className="text-sm text-foreground">{resolvedContent.address}</span>
                </div>
              </li>
              <li className="group flex items-start gap-3">
                <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-primary/10 transition-colors group-hover:bg-primary/20">
                ...
                <div>
                  ...
                  <span className="text-sm text-foreground" dir="ltr">{resolvedContent.phone}</span>
                </div>
              </li>
              <li className="group flex items-start gap-3">
                <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-primary/10 transition-colors group-hover:bg-primary/20">
                ...
                <div>
                  ...
                  <span className="text-sm text-foreground">{resolvedContent.email}</span>
                </div>
              </li>
            </ul>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-3">
              {resolvedContent.socialLinks.map((social) => (
                <Link
      ...
      <div className="border-t border-border/50">
        <div className="container mx-auto flex flex-col items-center justify-between gap-4 px-4 py-6 md:flex-row">
          <p className="text-sm text-muted-foreground">{resolvedContent.copyright}</p>
          <div className="flex items-center gap-6">
            <Link href="#" className="text-sm text-muted-foreground transition-colors hover:text-primary">
              {resolvedContent.privacyLabel}
            </Link>
            <Link href="#" className="text-sm text-muted-foreground transition-colors hover:text-primary">
              {resolvedContent.termsLabel}
            </Link>
          </div>
        </div>
      </div>
