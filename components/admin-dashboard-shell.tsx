...
import { Bell, ChevronDown, Download, FolderPlus, MapPinned, Menu, Plus, Upload, UserRound } from "lucide-react"
...
type SidebarBadgeCounts = {
  ...
  pendingStaffRequestsCount: number
}

const SIDEBAR_BADGES_TTL_MS = 30_000
let sidebarBadgesCache: { value: SidebarBadgeCounts; expiresAt: number } | null = null
let sidebarBadgesRequest: Promise<SidebarBadgeCounts> | null = null

async function fetchSidebarBadgeCounts(permissions: Array<DashboardPermissionKey | "*">, forceRefresh = false): Promise<SidebarBadgeCounts> {
  const now = Date.now()

  if (!forceRefresh && sidebarBadgesCache && sidebarBadgesCache.expiresAt > now) {
    return sidebarBadgesCache.value
  }

  if (!forceRefresh && sidebarBadgesRequest) {
    return sidebarBadgesRequest
  }

  sidebarBadgesRequest = (async () => {
    ...
    const value = {
      pendingTasksCount: tasksPayload?.pendingTasksCount ?? 0,
      pendingTransactionsCount: transactionsPayload?.unreadNotificationsCount ?? 0,
      pendingStaffRequestsCount: administrativePayload?.unreadReviewRequestsCount ?? 0,
    }

    sidebarBadgesCache = {
      value,
      expiresAt: Date.now() + SIDEBAR_BADGES_TTL_MS,
    }
  ...
  try {
    return await sidebarBadgesRequest
  } finally {
    sidebarBadgesRequest = null
  }
}
...
const ProfileInfoItem = memo(function ProfileInfoItem({ label, value }: { label: string; value: string }) {
  return (
...
const SidebarContent = memo(function SidebarContent({ permissions, logoUrl, logoAlt, logoArabicName, logoEnglishName, logoTextColor, logoArabicFontWeight, logoEnglishFontWeight }: { permissions: Array<DashboardPermissionKey | "*">; logoUrl: string; logoAlt: string; logoArabicName: string; logoEnglishName: string; logoTextColor: string; logoArabicFontWeight: "normal" | "bold"; logoEnglishFontWeight: "normal" | "bold" }) {
  ...
  useEffect(() => {
    ...
    async function loadSidebarBadges(forceRefresh = false) {
      try {
        const nextCounts = await fetchSidebarBadgeCounts(permissions, forceRefresh)

        if (!isCancelled) {
          setPendingTasksCount(nextCounts.pendingTasksCount)
          setPendingTransactionsCount(nextCounts.pendingTransactionsCount)
          setPendingStaffRequestsCount(nextCounts.pendingStaffRequestsCount)
        }
      } catch {
  ...
  return (
    <div className="flex h-full flex-col bg-white">
      <div className="flex justify-end border-b border-border/60 px-3 py-4 text-right">
        {isMounted && logoUrl ? (
          <div className="inline-flex max-w-full items-center justify-end gap-3">
          ...
          <div className="h-14 w-[220px]" aria-hidden="true" />
        )}
      </div>

      <div className="flex-1 overflow-y-auto px-3 py-3 text-right">
        {sections.map((group) => {
...
export function AdminDashboardShell({ userId, userName, userTitle, userProfileSummary, userPermissions, logoUrl, logoAlt, logoArabicName, logoEnglishName, logoTextColor, logoArabicFontWeight, logoEnglishFontWeight, children }: AdminDashboardShellProps) {
  ...
  useEffect(() => {
    if (pathname !== "/dashboard/my_files") {
      setDriveActionsState({ canUpload: false, canCreateFolder: false, googleEmail: null, canDisconnect: false, canChooseLocation: false })
      return
    }
  ...
  const isOperationalPlansPage = pathname === "/dashboard/operational_plans"
  const isAdministrativeBalancesPage = pathname === "/dashboard/administrative_balances"
  const isSupportersPage = pathname === "/dashboard/supporters"
  ...
  return (
    <div dir="rtl" className="dashboard-rtl min-h-screen bg-[linear-gradient(180deg,#f8fbfb,#eef5f5)] text-right">
      <div className="flex min-h-screen w-full items-start lg:flex-row">
        <aside className={isSidebarCollapsed ? "hidden" : "sticky top-0 hidden h-screen w-[272px] shrink-0 border-l border-white/60 bg-white/95 shadow-[10px_0_35px_rgba(15,23,42,0.04)] lg:block"}>
          ...
          <div className="w-full">
            <div className="mb-6 flex w-full items-center justify-between rounded-[2rem] border border-white/70 bg-white/90 px-5 py-4 shadow-[0_18px_50px_rgba(15,23,42,0.06)] backdrop-blur-sm">
              <Button
                ...
                variant="outline"
                size="icon"
                className="hidden rounded-2xl border-primary/15 bg-white/80 lg:inline-flex"
              ...
              <div className="flex items-center gap-3">
                {isAdministrativeBalancesPage ? (
                  <>
                    <Button
                      type="button"
                      variant="outline"
                      className="rounded-2xl border-primary/15 bg-white/80"
                      onClick={() => window.dispatchEvent(new Event("administrative-requests-open-global-balances"))}
                    >
                ...
                {isSupportersPage ? (
                  <>
                    <Button
                      ...
                      variant="outline"
                      className="rounded-2xl border-primary/15 bg-white/80"
                      onClick={() => window.dispatchEvent(new Event("supporters-export-excel"))}
                    >
                    ...
                    <Button
                      ...
                      variant="outline"
                      className="rounded-2xl border-primary/15 bg-white/80"
                      onClick={() => window.dispatchEvent(new Event("supporters-import-excel"))}
                    >
                ...
                {isOperationalPlansPage ? (
                  <Button
                    type="button"
                    variant="outline"
                    className="rounded-2xl border-primary/15 bg-white/80"
                    onClick={() => window.dispatchEvent(new Event("operational-plans-open-create-plan"))}
                  >
                    <Plus className="h-4 w-4" />
                    إضافة خطة
                  </Button>
                ...
                <Sheet>
                  ...
                  <SheetContent side="left" className="w-[272px] border-r border-border/60 p-0 text-right sm:max-w-[272px]">
            ...
            <div className={isSidebarCollapsed ? "w-full px-1 md:px-2" : "mx-auto w-full max-w-[1280px] px-1 md:px-2"}>
              {children}
            </div>
          </div>
